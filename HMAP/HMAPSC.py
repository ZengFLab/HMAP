...
from SURE.utils import convert_to_tensor, tensor_to_numpy, CustomDataset, CustomDataset2, CustomDataset4
...
class HMAPSC(nn.Module):
    ...
    def _l1_embedding(self,xs,zs=None,metacell_view=False):
        if metacell_view:
    ...
    def get_l1_embedding(self, xs, zs=None, batch_size=1024, metacell_view=False):
        ...
        with tqdm(total=len(dataloader), desc='', unit='batch') as pbar:
            for X_batch, Z_batch, _ in dataloader:
                if zs is None:
                    Z_batch = None
                zns = self._l1_embedding(X_batch, zs=Z_batch, metacell_view=metacell_view)
    ...
    def metacell_similarity(self, xs, ys=None):
        if ys is None:
    ...
    def metacell_network(self, xs, ys=None, k=10, exclude_metacells: list = None):
        affinity_matrix = self.metacell_similarity(xs, ys)
        self.G = nx.Graph()
...
def main():
    ...
    if args.save_model is not None:
        HMAPSC.save_model(hmapsc, args.save_model)
